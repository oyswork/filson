...
pub trait Extractable {
    ...
    ///     }
    ///
    ///     let cars = vec![Car{ make: "Ford", age: 7, milage: 100000 }, Car{ make: "Volvo", age: 3, milage: 50000 }, Car{make: "Volvo", age: 4, milage: 47000}];
    ///
    ///     // let's implement the Extractable so, that Filson knows how to extract age and make data from Car structs
    ...
    ///     let filtered_cars = cars.into_iter().filter(|car| flt.apply(car).unwrap_or(false)).collect::<Vec<_>>();
    ///     assert_eq!(filtered_cars, vec![ Car{ make: "Volvo", age: 3, milage: 50000 }, Car{make: "Volvo", age: 4, milage: 47000}]);
    /// ```
    fn extract(&self, path: &str) -> Result<DataNode, FilsonError>;
}
